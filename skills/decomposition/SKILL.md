---
name: decomposition
description: Use when breaking **approved scope** into work units at the right granularity — epics, stories, and sprints — and validating them before they reach development. Fire when the user wants to decompose a PRD into epics, break an epic into stories, organize stories into sprints, review/cut a backlog, run sprint planning, or check whether stories are READY-FOR-DEV. Trigger phrases: "decompose", "break this down", "epics", "stories", "user stories", "backlog", "sprint planning", "is this epic well-cut", "review these stories", "validate the decomposition", "READY-FOR-DEV check". This skill defines **what** a good cut looks like and the validation verdict. For the parallel sub-agent squad that produces a decomposition across multiple specialist perspectives, use `planning-squad` (the two compose: decomposition is the standard, planning-squad is one way to produce it). NOT for implementing a story that is already written — use `dev`. NOT for authoring the PRD itself — assumes scope is already approved (use `prd-writing`).
---
