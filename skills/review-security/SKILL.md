...
description: Use when reviewing **already-written code or a diff** for exploitable security defects — injection, broken authn/authz (incl. IDOR), hardcoded or leaked secrets, unsafe input handling, crypto misuse, and vulnerable dependencies. Fires on direct asks like "security review", "check this for vulnerabilities", "is this safe?", "can this be abused?", or when the `code-review` umbrella dispatches its security pass. One rigid lens with an Iron Law: a real finding blocks the merge and is never downgraded to PASS. Does NOT judge performance, style, or correctness. Distinguish from siblings: `security-requirements` is requirements-time (define what "secure" means before code exists); `threat-modeling` is design-time (system-level attack surfaces and trust boundaries); `standards-security` authors the project's `security.md` checklist that this skill consumes. If the user is asking about code that already exists → this skill. If they're asking what to require, how to design, or how to write the standard → route to the sibling.
---
