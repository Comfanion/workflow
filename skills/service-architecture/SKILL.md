...
description: Design and document the internal architecture of ONE service or application — choose its internal style (Layered, Hexagonal, Clean, Vertical Slices), apply patterns (DDD, CQRS, Event Sourcing) where a requirement demands, define internal module boundaries and data ownership, record technology decisions, and validate the design against the PRD and NFRs. Use this whenever the user wants to design how a single service/app is built inside, define its modules, pick its internal style or stack, or review/validate a service's architecture, or mentions "service architecture", "internal design", "module boundaries", "layered", "hexagonal", "clean architecture", "DDD", "CQRS", or "technology stack". This is the SERVICE altitude — for how multiple services fit together (the landscape, inter-service contracts, topology), use system-architecture instead; for one module's detailed contract, use unit-writing.
---
