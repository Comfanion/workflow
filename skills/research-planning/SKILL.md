...
description: Surface what is NOT yet understood — unknowns, risky assumptions, open questions that block a confident decision — and turn them into a prioritized plan of timeboxed spikes, each with a sharp question, an exit criterion, a timebox, and an owner. Use whenever the user says "we don't know yet", wants to "plan research", run a "spike", "investigate this", lists "open questions" or "unknowns", wants to "de-risk", or asks "what do we need to find out". This plans and scopes investigations; it does not perform the research (that is research-methodology) and does not make the final decision.
---
