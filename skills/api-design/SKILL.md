---
name: api-design
description: Use when **designing a SPECIFIC API contract** — one service's or one resource's endpoints, request/response shapes, status codes, error envelope, pagination, on-the-wire versioning, auth, and rate limiting — and producing its machine-readable spec (OpenAPI/Swagger YAML, GraphQL SDL, or `.proto`). Fire when the user wants to "design the API", "write the API contract", "define endpoints", "pick REST vs GraphQL vs gRPC vs WebSocket", "decide status codes / error format / pagination **for an endpoint**", "plan a v1→v2 version bump", "generate an OpenAPI spec", "write a `.proto`", or "design the GraphQL schema". This skill authors the contract artifact (e.g. `docs/architecture/{module}/api/{resource}.yaml`) and the design decisions behind it — protocol choice, naming, error table, auth model. **NOT** for authoring the PROJECT-WIDE API style guide (the canonical response envelope, the shared error-code table, the global rate-limit policy, global URL conventions) — that is `standards-api`; consume those standards here, don't rewrite them. **NOT** for documenting a module's API surface as ONE FILE inside a broader unit-docs folder (index + data model + API + events) — that is `unit-writing`; load this skill when the API contract itself is the deliverable and needs designing, not when it is a section of a module handbook. **NOT** for the internal architecture of the service that exposes the API (module boundaries, layering, tech stack) — that is `service-architecture`. Owns the wire contract only; business logic, data-model internals, and infrastructure live in their own documents.
---
