---
name: design-system
description: Use whenever the user wants to set up, evolve, or document the reusable VISUAL SYSTEM a whole product is built from — the foundation every feature composes instead of reinventing. Strong triggers: "design system", "design tokens", "component library", "style guide", "visual language", "theming / dark mode", "design foundations / primitives", or asking to define a color palette, typography scale, spacing scale, radii, shadows, or naming conventions; also when organizing scattered one-off styles into one consistent, themeable system. Covers the token layer (primitive → semantic), the component library with its variants and states, the visual language and naming scheme, theming and dark mode, and the consistency rules (tokens over literals, closed variant sets, accessible by construction). NOT for the per-feature experience — user flows, screen layouts, wireframes, interaction patterns, and per-feature UI states (empty/loading/error/success) belong to `ux-design`, which composes from the system defined here. Also not for the implementation code: this defines what the visual system is, not the components as code. Route here when the request names the shared palette/type/spacing scale or asks to "make this consistent/themeable"; route to `ux-design` when it names a specific feature's screens or how they work.
---
