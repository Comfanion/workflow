...
description: Design a relational database schema, plan safe migrations, and define an indexing strategy — model entities and relationships, normalize then denormalize with reason, enforce constraints at the database level, and write zero-downtime migrations. Use this whenever the user wants to design the database, write the DB schema, create migrations, define a data model, plan indexing, or mentions "schema", "migration", "indexing", or "query performance". For choosing the database engine itself (PostgreSQL vs MongoDB vs Redis, etc.), that is a research decision — use the research-methodology skill instead.
---
