---
name: test-design
description: Use when the user wants to design a test **strategy or plan** — not write individual concrete test cases. Fires on: "design a test strategy", "build a test plan", "what should I test", "what am I not testing", "test coverage gap", "which test level should I use" (unit vs integration vs E2E), "plan the test pyramid", "map acceptance criteria to tests", "what's the testing approach for this feature". This skill picks the levels, walks the four case classes (happy/error/edge/auth) per behavior, sets coverage targets, and maps every acceptance criterion to where it gets proven — i.e. it decides *what to test and at what level*. Do NOT use for: (a) authoring concrete enumerated test cases with specific inputs and expected outputs — that is `test-scenarios`; (b) running the test suite, collecting results, or reporting pass/fail — that is `test-execution`; (c) debugging one specific failing or flaky test — that is `systematic-debugging`. If the user already has a strategy and wants the actual case list written, hand off to `test-scenarios`.
---
