...
description: Use whenever a bug, failure, or unexpected behavior surfaces — a failing test, a crash, a panic, a build break, a wrong return value, a flaky/intermittent failure, or any "this isn't working" report. Cross-cutting: invoke it **inside** `dev` when a test won't pass, **inside** `test-execution` when a case fails, anywhere code misbehaves — even if already mid-loop in another skill, because diagnosing a failure is a distinct discipline from writing features. Iron Law is absolute: **no fix without root-cause investigation first.** Read the full error and stack trace, reproduce reliably, trace the bad value back to its source, form one hypothesis, test it minimally, fix at the source, then prove the fix with a reproduction. "I see the problem, let me just change this" is the exact thought this skill exists to stop — a symptom is not a cause. Use it **especially** when: a previous fix did not hold (the bug returned = the prior fix was wrong, re-map from scratch), you feel tempted by a quick patch, or you are about to try a second or third fix attempt — three failed fixes means stop and question the architecture, not a fourth guess. Do NOT use for: writing new features or adding capabilities (`dev`); designing or authoring the test suite (`test-execution` surfaces failures, this skill diagnoses them); or reviewing completed work (`review-correctness`).
---
