---
name: test-execution
description: Use when the user wants to run or execute tests, produce a test report, file a defect with repro steps, or obtain a shipping verdict. This skill owns **two gates** and emits evidence for both: (1) the **integration gate** — the build command exits 0 AND the full test suite exits 0, with output quoted from the current turn, checked before a story can move to `review`; (2) the **QA gate** — a binary PASS/FAIL verdict backed by what ran, what passed, what is open, and which acceptance criteria are proven. Fires on: "run the tests", "execute the test plan", "run the test suite" / "run all tests", "verify the build is green", "qa gate", "test report", "record pass/fail results", "file a defect with repro steps", "is this ready to ship" (when tests are expected to back the answer). Records pass/fail/blocked/skipped against existing case ids and files defects with expected-vs-actual, severity, and quoted evidence; never issues a soft yes or a PASS-with-caveats. Does NOT author test cases (that is `test-scenarios`). Does NOT diagnose root cause or debug failures (that is `systematic-debugging` — this skill only reports and records defects, it does not explain or fix them). Does NOT make the broad "is all the work complete" call (that is `verification-before-completion`, which reads the QA verdict this skill produces as one of its inputs). Does NOT produce the build or compile the code (that is the dev role — this skill consumes the build and checks it is green).
---
