---
name: test-scenarios
description: Author thorough, concrete test cases and scenarios with real input data, covering specific and edge cases — especially the non-obvious ones that surface while code is being written. Use whenever the user wants "test cases", "test scenarios", "edge cases", asks "what should I test", says "cover this case", or needs "negative tests" or "boundary cases". This enumerates the concrete cases to run; it does not decide the test strategy/levels (that is test-design) and does not write the Given/When/Then acceptance criteria the scenarios trace to (that is acceptance-criteria).
---
