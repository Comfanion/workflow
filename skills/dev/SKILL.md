---
name: dev
description: Implement approved stories with a study-first, test-driven loop — design interfaces, write the failing test, make it green, refactor, review. Use this whenever the user wants to "implement this story", "build the epic", "work the sprint", "implement", "code this up", or asks for "TDD". Handles a single story (the core loop) and the two batch modes: a whole epic (N stories) and a whole sprint (N epics). This skill executes stories that are already written and approved — it does not author or decompose them (that is the decomposition skill's job).
---
