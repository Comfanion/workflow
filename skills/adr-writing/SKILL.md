---
name: adr-writing
description: Use when the user wants to record, document, or update a significant architecture decision — capturing the rationale so it survives. Typical trigger phrasings: "write an ADR", "record this architecture decision", "document why we chose X over Y", "capture the rationale for X", "supersede / deprecate / replace ADR-NNN", "add to the decision log", or any mention of "ADR", "architecture decision record", "decision log", or "technical decision record". Fires for choices with long-term, hard-to-reverse consequences: database, framework, architecture pattern (event-driven, CQRS), API design (versioning, contract shape), major library with lock-in. Also fires for the full lifecycle: proposing, accepting, superseding, or deprecating an existing ADR (create the new record, link the two, update statuses). Anti-triggers — use the sibling skill instead: for the broader system or service design that a decision fits into, use `system-architecture` / `service-architecture` (an architecture doc names a pattern and links its ADR rather than re-deciding it inline); for a recurring rule applied across the codebase (naming, formatting, error handling), use `authoring-standards` (a standard cites its governing ADR; the ADR holds the why, the standard states the rule); for following or consuming an existing standard, use `using-standards`. Does NOT fire for minor utility choices, code style, requirements, or task breakdowns — redirect those rather than producing a record. An ADR captures exactly one decision: the forces and constraints, the options weighed, the decision, and its consequences.
---
