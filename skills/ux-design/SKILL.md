---
name: ux-design
description: Use when the user wants to design the UX or experience of a feature: map a user flow or user journey, produce wireframes or low-fidelity screen layouts, define interaction design (triggers, transitions, input/validation behavior), specify UI states (empty, loading, error, success), cover accessibility/WCAG or responsive behavior for a screen, or prepare a design handoff a frontend implementer can build from without re-asking. Trigger phrases: "design the UX", "design this feature's experience", "user flow", "user journey", "wireframe", "screen design", "screen layout", "interaction design", "design handoff", "spec the screen", "what states does this need", "empty/loading/error state", "a11y for this screen", "WCAG for this screen", "responsive layout", "how does this behave on mobile". This skill defines the EXPERIENCE per feature — flows, screens, states, interactions, accessibility, responsive behavior, and the buildable handoff spec. It does NOT define the reusable visual system: design tokens, the component library, theming, color palette, typography scale, spacing, icon set, and naming conventions belong to `design-system`. It does NOT write implementation code — it stops at the spec the implementer builds from.
---
