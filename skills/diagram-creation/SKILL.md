...
description: Create or update technical diagrams that document software — C4 architecture (context/container/component), sequence flows, ER data models, flowcharts, and state machines — in Mermaid or ASCII. Use this whenever the user wants to "create a diagram", "draw the architecture", "visualize a flow", "show how the system fits together", or mentions "C4 diagram", "sequence diagram", "ER diagram", "flowchart", "state diagram", or "mermaid". This skill produces the diagram artifacts only — it does not make the architectural decisions those diagrams illustrate (that belongs to the architecture/design work).
---
