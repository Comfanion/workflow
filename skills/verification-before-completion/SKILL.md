...
description: Gate any "done / fixed / passing / ready / works" claim behind fresh evidence — run the command that proves the claim, read its output and exit code, and only then state the result. Use this whenever you are about to say work is complete, a bug is fixed, tests pass, a build succeeds, or before committing, pushing, opening a PR, or handing off to another agent or role. This is the completion gate that every role passes through — it does not decide what to build, review, or test; it decides whether you have earned the right to call something done.
---
