...
description: Use this whenever you — the author or developer — receive code-review feedback (PR/MR comments, change requests, reviewer notes, automated or AI-generated review findings) and must decide how to act on each item. The discipline is universal, not selective: understand every comment, verify it against the code before changing anything, implement the ones that hold, push back with evidence (`path:line`, test, doc) on the ones that don't, and ask when a comment is unclear. This applies to *all* received feedback — including nits, benign-looking comments, and cases where you were told to "just fix all of them." Reflexive compliance is the failure mode this skill exists to prevent, so it fires even when the instruction sounds like a simple fix-all; verification comes first, always. Do NOT use this to produce review feedback on someone else's code — that is `code-review`, the counterpart that writes the findings. Do NOT use it to write original code or implement features from scratch — that is `dev`. One-line boundary: `code-review` emits the comments; this skill decides what to do with each one without rubber-stamping or stonewalling.
---
