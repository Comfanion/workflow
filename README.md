...
- **Roles (`agents/`)** are viewpoints — secretary, orchestrator, analyst, architect, pm, designer, fullstack/backend/frontend-developer, tester, reviewer, researcher, devops. Each is a perspective with a mission and a scope. A role does **not** own a fixed set of skills, and it never references another role; it just knows its own job.
